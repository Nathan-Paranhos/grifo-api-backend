...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (error) {
        logger.error('Contests list error:', error);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      logger.debug('Contest retrieved', {
        userId: user.id,
        contestId: id,
        userRole: user.role,
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (error) {
        logger.error('Contest resolution error:', error);
        throw new AppError('Erro ao resolver contestação');
      }

      logger.info('Contest resolved', {
        userId: user.id,
        contestId: id,
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (error) {
        logger.error('Contest reopen error:', error);
        throw new AppError('Erro ao reabrir contestação');
      }

      logger.info('Contest reopened', {
        userId: user.id,
        contestId: id,
        inspectionId: existingContest.inspection_id,
        reopenReason
