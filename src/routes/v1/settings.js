...
const updateCompanySettingsSchema = z.object({
  ...
  branding_settings: z.object({
    ...
    primary_color: z.string().regex(/^#[0-9A-Fa-f]{6}$/, 'Cor deve estar no formato #RRGGBB').optional(),
    secondary_color: z.string().regex(/^#[0-9A-Fa-f]{6}$/, 'Cor deve estar no formato #RRGGBB').optional(),
...
const updateUserSettingsSchema = z.object({
  ...
  inspection_preferences: z.object({
...
router.get('/company', 
  ...
  asyncHandler(async (req, res) => {
    ...
    if (error) {
      logger.error('Erro ao buscar configurações da empresa', { error, company_id: req.tenant.id });
      throw error;
    }
...
router.put('/company',
  ...
  asyncHandler(async (req, res) => {
    ...
    if (fetchError) {
      logger.error('Erro ao buscar configurações atuais', { error: fetchError });
      throw fetchError;
    }
    ...
    if (error) {
      logger.error('Erro ao atualizar configurações da empresa', { error, company_id: req.tenant.id });
      throw error;
    }
...
router.get('/user', asyncHandler(async (req, res) => {
  ...
  if (error) {
    logger.error('Erro ao buscar configurações do usuário', { error, user_id: req.user.id });
    throw error;
  }
...
router.put('/user',
  ...
  asyncHandler(async (req, res) => {
    ...
    if (fetchError) {
      logger.error('Erro ao buscar configurações atuais do usuário', { error: fetchError });
      throw fetchError;
    }
    ...
    if (error) {
      logger.error('Erro ao atualizar configurações do usuário', { error, user_id: req.user.id });
      throw error;
    }
    
    logger.info('Configurações do usuário atualizadas', {
      user_id: req.user.id,
      changes: Object.keys(req.body)
    });
