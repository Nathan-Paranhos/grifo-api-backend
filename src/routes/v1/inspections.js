...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!canUpdate) {
      throw new AuthorizationError('Sem permissão para atualizar esta vistoria');
    }
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    logger.info('Inspection status updated successfully', {
      inspectionId: id,
      newStatus: status,
      updatedBy: userId,
      empresaId
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (error) {
      logger.error('Error adding inspection item:', error);
      throw new AppError('Erro ao adicionar item à vistoria', 500);
    }

    logger.info('Inspection item added successfully', {
      itemId: item.id,
      inspectionId: id,
      createdBy: userId
    });
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    logger.info('Inspection item updated successfully', {
      itemId,
      inspectionId: id,
      updatedBy: userId
    });
