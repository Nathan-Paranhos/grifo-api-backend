...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (syncError) {
        logger.error('Sync status error:', syncError);
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (!force) {
        ...
        if (checkError) {
          logger.error('Active sync check error:', checkError);
      ...
      if (createError) {
        logger.error('Sync operation creation error:', createError);
      ...
      setTimeout(async () => {
        try {
          ...
          logger.error('Sync simulation error:', error);
        }
      }, 1000);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      logger.debug('Sync history retrieved', {
        userId: user.id,
        companyId: user.company_id,
        count,
        page,
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      logger.debug('Sync operation retrieved', {
        userId: user.id,
        syncId: id,
      ...
      logger.error('Sync operation get error:', error);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (updateError) {
        logger.error('Sync cancellation error:', updateError);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (updateError) {
        logger.error('Sync retry error:', updateError);
      ...
      setTimeout(async () => {
        try {
          ...
          logger.error('Sync retry simulation error:', error);
        }
      }, 1000);
...
async function simulateSyncProcess(syncId, _syncType, _entityTypes) {
  try {
    ...
    logger.error('Sync process simulation error:', error);
